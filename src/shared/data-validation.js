  ...
  function validate(schema, data, required = true) {
    ...
    for (const group of schema.groups || []) for (const field of group.fields || []) {
      ...
      if (field.type === 'array') {
        ...
        if (!Array.isArray(value) || (field.rows != null && value.length !== field.rows) || value.some(r => !Array.isArray(r) || (field.cols != null && r.length !== field.cols))) { errors.push(label + '：矩阵维度错误'); continue; }
        values = value.flat();
      }
