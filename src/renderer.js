...
// ── 原始数据照片自动嵌入（默认开：没设过就是开）──
function photoEmbedEnabled() {
  return loadSettings().embedPhoto !== false;
}
...
async function pumpQueue() {
  ...
  for (const q of genQueue) {
    ...
    try {
      const r = await window.labAPI.runGenerate(q.exp.path, studentInfo, null, expOverrides(q.exp.id), photoEmbedEnabled());
...
function refreshFormCheck() {
  ...
  for (const group of (currentSchema.groups || [])) {
    for (const fld of (group.fields || [])) {
      ...
      if (Array.isArray(v)) {
        // 定长容器（matrix 且无 rowLabels）：整行全空 = 没用到的格子，不算缺失。
        // 长度是硬要求的（array、以及有 rowLabels 的固定表）保持原来的严格判定。
        if (fld.type === 'matrix' && !fld.rowLabels) {
          const filled = v.filter(r => Array.isArray(r) && r.some(x => x !== null && x !== undefined));
          const half = filled.some(r => r.some(x => x === null || x === undefined));
          if (!filled.length || half) missing.push(fld.label || fld.key);
        } else {
...
// AI 面板里"这次用哪几个技能"的勾选偏好（与上面的"启用哪些技能"是两回事）
function getSkillSelection() {
  const s = loadSettings();
  return (s.skillSelection && typeof s.skillSelection === 'object') ? s.skillSelection : {};
}
function setSkillSelection(id, checked) {
  const s = loadSettings();
  s.skillSelection = Object.assign({}, getSkillSelection());
  s.skillSelection[id] = !!checked;
  saveSettings(s);
}

// ── 提示词公共块（AI 润色与 AI 变体共用同一份，避免两处措辞各自漂移）──
// 知识库硬性约束块：kbOnly 关掉时返回空串（= 不约束）。
async function buildKbBlock(expPath, kbOnly) {
  if (!kbOnly) return '';
  let ragText = '';
  try {
    const rr = await window.labAPI.readRag(expPath);
...
// 技能指令块。不传 skillIds 时取「设置 → AI 润色技能」里启用的那些 ——
// AI 变体生成没有勾选框，直接吃这份默认，这正是它该有的「基础设置」语义。
function buildSkillBlock(skillIds) {
  const ids = skillIds || getEnabledSkills().map(s => s.id);
  return ids
...
function renderSkillOptions(keepValue) {
  ...
  if (!box) return;
  // keepValue：面板上已有勾选状态就以它为准（重渲染不丢当前这次的选择）；
  // 否则读持久化偏好，**没有偏好就默认勾上** —— 技能是我们的基础设置，
  // 一个"要记得手动开"的技能等于没有（原先首次渲染一律不勾，等于静默失效）。
  //
  // 判定"已有状态"必须看 DOM 里有没有 checkbox，而不是只看 keepValue 这个开关：
  // loadSkillList() 首次加载传的就是 true，那时框还是空的 —— 只看标志位会走成
  // "空集 → 全不勾"，正是要修的那个 bug。
  const hasExisting = box.querySelector('input[type="checkbox"]') !== null;
  const prev = (keepValue && hasExisting)
    ? new Set([...box.querySelectorAll('input:checked')].map(i => i.value))
    : null;
  const saved = getSkillSelection();
  box.innerHTML = '';
  ...
  for (const sk of skills) {
    ...
    cb.type = 'checkbox';
    cb.value = sk.id;
    cb.checked = prev ? prev.has(sk.id) : (saved[sk.id] !== false);
    cb.onchange = () => setSkillSelection(sk.id, cb.checked);
    label.appendChild(cb);
...
async function runAiPolish() {
  ...
  // 知识库与技能指令：本轮所有润色对象共用
  const kbBlock = await buildKbBlock(currentExp.path, kbOnly);
  const skillBlock = buildSkillBlock(skillIds);
...
function loadSettingsForm() {
  ...
  $('chkEmbedPhoto').checked = s.embedPhoto !== false;
...
function saveAppSettings() {
  ...
  settings.embedPhoto = $('chkEmbedPhoto').checked;
...
async function runGenerateReport(btn, genExpId) {
  ...
  const result = await window.labAPI.runGenerate(genExp.path, studentInfo, variantChoices, expOverrides(genExp.id), photoEmbedEnabled());
  if (result.cancelled) {
    $('logContent').textContent += '\n⏹ 生成已取消\n';
...
async function runVariantAI() {
  ...
  try {
    // 技能与知识库跟着变体生成一起走：新变体是「照我们那套规范写出来的」，
    // 而不是照学长原版措辞改的。无 skillIds → 用设置里启用的技能（默认全开）。
    const skillBlock = buildSkillBlock();
    const kbBlock = await buildKbBlock(currentExp ? currentExp.path : '', $('chkKbOnly').checked);
    const messages = [
      {
        role: 'system',
        content: `你是大学物理实验报告写作助手。根据用户的调整指令，对给定的实验章节文本进行改写：保持物理原理与实验事实正确；必须原样保留 $...$ 公式和 %%DATA:xxx%% 数据占位符；使表达更具个人特色、避免与原文措辞重复。只输出改写后的完整文本，不要输出任何解释或前后缀说明。${skillBlock}${kbBlock}`,
      },
      {
        role: 'user',
...
// ══════════════════════════════════════════════════════════
// 数据表照片识别（纸质表格 → 表单）
// 流程：选图 → 压缩 → 视觉模型结构化输出 → 确定性校验 → 逐字段勾选 → 合并进表单
...
async function startRecognition(dataUrl, name) {
  ...
  const provider = (vProvider === 'inherit') ? (settings.provider || 'deepseek') : vProvider;
  // 非 inherit 的专用提供商（siliconflow / bailian 等）走预设的 URL 和模型兜底
  const model = settings.visionModel
    || (vProvider === 'siliconflow' ? 'Qwen/Qwen3-VL-32B-Instruct'
      : vProvider === 'bailian' ? 'qwen3-vl-plus'
      : settings.model)
    || '';
  const apiUrl = (vProvider === 'inherit') ? settings.apiUrl : '';

  let result;
  try {
    result = await window.labAPI.aiChat({
      provider, apiKey: settings.apiKey, apiUrl, model,
