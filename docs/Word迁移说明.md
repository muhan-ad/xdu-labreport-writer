...
| | 整改前 | 整改后 |
| --- | --- | --- |
| 全文无 ± | 9 个实验 | 8 个（RLC、光的偏振、理想气体、伏安特性、电容与高电阻、电表改装、重力加速度、霍尔效应） |
| ± 少于数值不确定度式 | 3 个 | 0 个 |
| 纯符号公式 >5 条 | 8 个 | 8 个（多数是原理式，如 `b=-R_g`，不都需要数值代入，需人看） |

已按样板整改 6 个实验：**拉伸法**（含物理 bug：Y 原先用未修正直径）、**直螺线管**、**激光波长**、
**长度与体积**、**灵敏电流计**、**低电阻**。剩余实验多在作图法/拟合类，要不要给 ± 、怎么给，
需要按各自物理量判断，留给分工核对。
