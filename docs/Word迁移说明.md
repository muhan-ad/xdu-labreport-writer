...
1. **不确定度呈现**：算了 `u(x)=…`（后面跟数字）却没有 ± → 结果没写成标准形式；
   `±` 条数少于 `u(x)=` 式条数 → 只补了一部分。
2. **计算过程**：只看**「二、数据处理」段内**的公式——原理/方法段的符号式（`pV=nRT`、`Q=CU`、
   `dsinφ=Kλ`）本就不该有数字，按段落区分后才有意义；数据处理段里一条带数字的公式都没有，
   才说明只给了符号式（初版把原理式也算进去，误报了 8 个实验）。

2026-09 基线（26 个实验）：

| 项 | 结果 |
| --- | --- |
| 数据处理段无一处数值代入 | **0 个**（原先误报 8 个，实为原理式） |
| ± 少于数值不确定度式 | 0 个 |
| 全文无 ± | 8 个（RLC、光的偏振、理想气体、电容与高电阻、电表改装、重力加速度、霍尔效应 + 已修中的个别） |

已整改 7 个实验：**拉伸法**（含物理 bug：Y 原先用未修正直径）、**直螺线管**、**激光波长**、
**长度与体积**、**灵敏电流计**、**低电阻**、**伏安特性**（线性电阻由拟合斜率给出 R = (99.9 ± 0.3) Ω）、
**静电场**（补拟合系数代回与电极几何反推的数值代入）。

剩下 8 个「无 ±」实验的共同点：脚本里**完全没有不确定度计算**，要补就得先定方法。
其中大多数有重复测量（A 类可算），但没有仪器误差字段（B 类需要新增 schema 字段或约定取法），
该不该给 ±、按哪种方法评，属于物理判断，留待分工核对。
