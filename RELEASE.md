...
- **感谢声明名单**：管理应用「感谢声明」写入主仓库 `物理实验/实验脚本/common/credits.json`，用户端「设置 → 感谢声明」读取。它与 common/ 其余文件走同一通路（应用启动时同步出厂文件、数据包更新时整目录替换），因此**改完名单同样要点一次「发布数据更新」或「推送实验包」才对用户端生效**；名单弹窗会提示「本地名单与线上不一致」。
